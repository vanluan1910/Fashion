...
export class CollapsibleContainerNode extends ElementNode {
  ...
  collapseAtStart(selection: RangeSelection): boolean {
    ...
    // Stub: $rewindSiblingCaret and $getSiblingCaret are not available in current Lexical version
    // Using alternative approach to insert nodes before this node
    const previousSibling = this.getPreviousSibling();
    if (previousSibling && $isElementNode(previousSibling)) {
      // Insert nodes as children of previous sibling
      previousSibling.append(...nodesToInsert);
    } else {
  ...
  static importJSON(
    ...
    // ElementNode properties are already set by $createCollapsibleContainerNode
    // No need for updateFromJSON as it doesn't exist in ElementNode
    return node;
  }

...
// Stub for @lexical/react/LexicalNodeContextMenuPlugin (not available)
type NodeContextMenuOptionConfig = {
  $onSelect: () => void;
  $showOn?: (node: any) => boolean;
  disabled?: boolean;
  icon?: JSX.Element;
};

class NodeContextMenuOption {
  constructor(public label: string, public config: NodeContextMenuOptionConfig) {}
}

class NodeContextMenuSeparator {
  constructor(public config?: { $showOn?: (node: any) => boolean }) {}
}

function NodeContextMenuPlugin({ 
  className, 
  itemClassName, 
  separatorClassName, 
  items 
  ...
  // Stub: Context menu is not available
  return null;
}
import {
  $getSelection,

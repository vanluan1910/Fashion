...
// Stub for isModifierMatch (not available in current Lexical version)
export function isModifierMatch(
  ...
  // Check if at least one modifier is required
  const hasModifier = modifiers.ctrlKey !== undefined || modifiers.metaKey !== undefined || modifiers.shiftKey !== undefined || modifiers.altKey !== undefined;
  
  // If no modifiers specified (empty object), check if any modifier is pressed
  if (!hasModifier) {
  ...
  if (modifiers.shiftKey !== undefined && event.shiftKey !== modifiers.shiftKey) {
    return false;
  }
